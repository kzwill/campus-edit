...
define(['exports', './AttributeCompression-e6061e33', './Matrix2-a67cd174', './RuntimeError-8952249c', './defaultValue-81eec7ed', './ComponentDatatype-0f8fc942', './Transforms-fe4a2875', './EncodedCartesian3-bf2feda0', './GeometryAttribute-013ac666', './IndexDatatype-2261ba8d', './IntersectionTests-803c75e5', './Plane-8a0f393e'], (function (exports, AttributeCompression, Matrix2, RuntimeError, defaultValue, ComponentDatatype, Transforms, EncodedCartesian3, GeometryAttribute, IndexDatatype, IntersectionTests, Plane) { 'use strict';
  ...
  GeometryPipeline.projectTo2D = function (
    ...
    attributeName2D,
    projection,
    needsTranslateAmericaToEast = false // THING.EARTH2.0 huyang
  ) {
    //>>includeStart('debug', pragmas.debug);
    if (!defaultValue.defined(geometry)) {
      throw new RuntimeError.DeveloperError("geometry is required.");
    ...
    for (let i = 0; i < values3D.length; i += 3) {
      ...
      if (
        needsTranslateAmericaToEast &&
        lonLat.longitude < (-30 * Math.PI) / 180
      ) {
        lonLat.longitude += Math.PI * 2;
      }
      //>>includeStart('debug', pragmas.debug);
  ...
  // THING.EARTH2.0 zhangtengfei
  const tempWorldPositionArray = [];
  function combineGeometriesForTHINGEARTH(instances, propertyName, params) {
    const length = instances.length;
    ...
    const useMercatorUV = params.useMercatorUV;
    const mapProjection = params.mapProjection;
    const isAmericaInWest = params.isAmericaInWest;
    //huyang 20200316 是否将经纬度添加到geometry中
    const addLonlatToGeometry = params.addLonlatToGeometry;
    const Utils = params.Utils;

    if (Utils.getEarthMode() !== "3D") {
      for (let i = 0; i < length; i++) {
        const geometry = instances[i][propertyName];
        ...
        instances[i][propertyName].attributes["position"] =
          instances[i][propertyName].attributes["position2D"];
        delete instances[i][propertyName].attributes["position3D"];
        delete instances[i][propertyName].attributes["position2D"];
      }
    }
    ...
    let tempLocalPositionArray;

    //添加一个attribute 先把uv的数值给过去
    if (addLonlatToGeometry) {
      attributes.lonlat = new GeometryAttribute.GeometryAttribute({
        componentDatatype: ComponentDatatype.ComponentDatatype.DOUBLE,
        componentsPerAttribute: 3,
        values: new Float64Array(attributes.position.values.length),
      });
      instances[0][propertyName].attributes.lonlat = new GeometryAttribute.GeometryAttribute({
        componentDatatype: ComponentDatatype.ComponentDatatype.DOUBLE,
        componentsPerAttribute: 3,
        values: new Float64Array(
          instances[0][propertyName].attributes.position.values.length
        ),
      });
      instances[1][propertyName].attributes.lonlat = new GeometryAttribute.GeometryAttribute({
        componentDatatype: ComponentDatatype.ComponentDatatype.DOUBLE,
        componentsPerAttribute: 3,
        values: new Float64Array(
          instances[1][propertyName].attributes.position.values.length
        ),
      });
    }
    // let topPositionValues = new Float64Array(
    //   instances[0][propertyName].attributes['position'].values
    // );
    // let sidePositionValues;
    // //可能有无侧面的情况 例如GeoWater
    // if(instances[1]){
    //   sidePositionValues = new Float64Array(
    //     instances[1][propertyName].attributes['position'].values
    //   );
    // }
    // Combine attributes from each geometry into a single typed array
    for (name in attributes) {
      if (attributes.hasOwnProperty(name)) {
        ...
        for (i = 0; i < length; ++i) {
          ...
          sourceValuesLength = sourceValues.length;
          for (j = 0; j < sourceValuesLength; ++j) {
            if (name === "position") ; else if (name === "st") {
              if (j % 2 === 0.0) {
                if (i === 0) {
                  // 顶面
                  if (Utils.getEarthMode() === "3D") {
                    tempWorldPositionArray[0] =
                      instances[i][propertyName].attributes["position"].values[
                        (j / 2) * 3
                      ];
                    tempWorldPositionArray[1] =
                      instances[i][propertyName].attributes["position"].values[
                        (j / 2) * 3 + 1
                      ];
                    tempWorldPositionArray[2] =
                      instances[i][propertyName].attributes["position"].values[
                        (j / 2) * 3 + 2
                      ];
                  } else {
                  ...
                  let coords = Utils.convertWorldToLonlat(tempWorldPositionArray);
                  // tempLonlatArray = coords;
                  if (useMercatorUV) {
                    coords = Utils.convertLonlatToWebMercator(coords);
                  }
                  const u =
                    (coords[0] - extent.minX) / (extent.maxX - extent.minX);
                  const v =
                    1 - (extent.maxY - coords[1]) / (extent.maxY - extent.minY);
                  // if (u < 0) {
                  //   u = 0;
                  // }
                  // if (u > 1) {
                  ...
                  // }
                  sourceValues[j] =
                    (u * (extent.maxX - extent.minX)) / textureSize[0];
                  sourceValues[j + 1] =
                    (v * (extent.maxY - extent.minY)) / textureSize[1];
                }
              }
            }
            //huyang  20230316 这里可以优化 先实现功能
            else if (name === "lonlat") {
              if (j % 3 === 0.0) {
                if (i === 0) {
                  if (Utils.getEarthMode() === "3D") {
                ...
                const coords = Utils.convertWorldToLonlat(tempWorldPositionArray);
                sourceValues[j] = coords[0];
                sourceValues[j + 1] = coords[1];
                sourceValues[j + 2] = coords[2];
              }
            }
            values[k++] = sourceValues[j];
          }
        ...
        for (i = 0; i < length; ++i) {
          ...
          for (j = 0; j < sourceValuesLength; ++j) {
            if (name === "position") {
              if (j % 3 === 0.0) {
                if (Utils.getEarthMode() === "3D") {
                  tempWorldPositionArray[0] = sourceValues[j];
                  tempWorldPositionArray[1] = sourceValues[j + 1];
                  tempWorldPositionArray[2] = sourceValues[j + 2];
                } else {
                  tempWorldPositionArray[0] = sourceValues[j];
                  tempWorldPositionArray[1] = sourceValues[j + 2];
                  tempWorldPositionArray[2] = -1 * sourceValues[j + 1];
                }

                tempLocalPositionArray = transform.worldToLocal(
                  tempWorldPositionArray
                );

                sourceValues[j] = tempLocalPositionArray[0];
                sourceValues[j + 1] = tempLocalPositionArray[1];
                sourceValues[j + 2] = tempLocalPositionArray[2];
              }
            }
            values[k++] = sourceValues[j];
          }
    ...
    if (haveIndices) {
      ...
      for (i = 0; i < length; ++i) {
        ...
        // const pA = new Cartesian3(), pB = new Cartesian3(), pC = new Cartesian3();
        // const nA = new Cartesian3(), nB = new Cartesian3(), nC = new Cartesian3();
        // const cb = new Cartesian3(), ab = new Cartesian3();
        for (k = 0; k < sourceIndicesLen; ++k) {
          destIndices[destOffset++] = offset + sourceIndices[k];

          // // 在此处重新计算法线，与T3D保持一致
          // if (k % 3 === 0) {
          //   // 预先设置后两位，为了获取一个三角面的三个索引，来计算法线
          //   destIndices[destOffset + 1] = offset + sourceIndices[k];
          //   destIndices[destOffset + 2] = offset + sourceIndices[k];

          //   // 获取索引值
          //   const vA = destIndices[k + 0];
          //   const vB = destIndices[k + 1];
          //   const vC = destIndices[k + 2];

          //   // 获取顶点坐标值
          //   Cartesian3.fromArray(instances[i][propertyName].attributes.position.values, vA * 3, pA);
          //   Cartesian3.fromArray(instances[i][propertyName].attributes.position.values, vB * 3, pB);
          //   Cartesian3.fromArray(instances[i][propertyName].attributes.position.values, vC * 3, pC);

          //   Cartesian3.subtract(pC, pB, cb);
          //   Cartesian3.subtract(pA, pB, ab);
          //   Cartesian3.cross(cb, ab, cb);

          //   Cartesian3.normalize(cb, cb);

          //   Cartesian3.fromArray(instances[i][propertyName].attributes.normal.values, vA * 3, nA);
          //   Cartesian3.fromArray(instances[i][propertyName].attributes.normal.values, vB * 3, nB);
          //   Cartesian3.fromArray(instances[i][propertyName].attributes.normal.values, vC * 3, nC);

          //   Cartesian3.add(nA, cb, nA);
          //   Cartesian3.add(nB, cb, nB);
          //   Cartesian3.add(nC, cb, nC);

          //   Cartesian3.normalize(nA, nA);
          //   Cartesian3.normalize(nB, nB);
          //   Cartesian3.normalize(nC, nC);

          //   Cartesian3.pack(nA, instances[i][propertyName].attributes.normal.values, vA * 3);
          //   Cartesian3.pack(nB, instances[i][propertyName].attributes.normal.values, vB * 3);
          //   Cartesian3.pack(nC, instances[i][propertyName].attributes.normal.values, vC * 3);
          // }
        }
  ...
  // THING.EARTH2.0 zhangtengfei 优化：在合并多边形几何体数据的时候，直接生成外部可用的转换/合并数据，外部不再进行循环遍历处理
  GeometryPipeline.combineInstancesForTHINGEARTH = function (instances, params) {
    //>>includeStart('debug', pragmas.debug);
    if (!defaultValue.defined(instances) || instances.length < 1) {
      throw new RuntimeError.DeveloperError(
        "instances is required and must have length greater than zero."
    ...
    for (let i = 0; i < length; ++i) {
      ...
      if (defaultValue.defined(instance.geometry)) {
        ...
        instanceSplitGeometry.push(instance);
        console.error("Polygon geometry data error!");
      }
    }
    ...
    if (instanceGeometry.length > 0) {
      geometries.push(
        combineGeometriesForTHINGEARTH(instanceGeometry, "geometry", params)
      );
    }

    if (instanceSplitGeometry.length > 0) {
      geometries.push(
        combineGeometriesForTHINGEARTH(
...
//# sourceMappingURL=GeometryPipeline-33c976f0.js.map
