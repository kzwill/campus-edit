...
define(['./defaultValue-81eec7ed', './Matrix2-a67cd174', './GeometryOffsetAttribute-2bff0974', './Transforms-fe4a2875', './RuntimeError-8952249c', './ComponentDatatype-0f8fc942', './GeometryAttribute-013ac666', './GeometryAttributes-32b29525', './GeometryInstance-8a3f1b2b', './GeometryPipeline-33c976f0', './IndexDatatype-2261ba8d', './PolygonPipeline-1f856727', './RectangleGeometryLibrary-c8165d46', './VertexFormat-7df34ea5', './_commonjsHelpers-3aae1032-26891ab7', './combine-3c023bda', './WebGLConstants-508b9636', './AttributeCompression-e6061e33', './EncodedCartesian3-bf2feda0', './IntersectionTests-803c75e5', './Plane-8a0f393e', './EllipsoidRhumbLine-d58fdd5b'], (function (defaultValue, Matrix2, GeometryOffsetAttribute, Transforms, RuntimeError, ComponentDatatype, GeometryAttribute, GeometryAttributes, GeometryInstance, GeometryPipeline, IndexDatatype, PolygonPipeline, RectangleGeometryLibrary, VertexFormat, _commonjsHelpers3aae1032, combine, WebGLConstants, AttributeCompression, EncodedCartesian3, IntersectionTests, Plane, EllipsoidRhumbLine) { 'use strict';
  ...
  RectangleGeometry.createGeometry = function (rectangleGeometry) {
    ...
    if (extrude) {
      ...
      geometry = constructRectangle(
        rectangleGeometry,
        computedOptions,
        rectangleGeometry.params
      );
      ...
      if (rectangleGeometry.params) {
        // eslint-disable-next-line no-undef
        if (THING.EARTH.Utils.getEarthMode() !== "3D") {
          GeometryPipeline.GeometryPipeline.projectTo2D(
            ...
            "position2D",
            // eslint-disable-next-line no-undef
            THING.Utils.getCurrentApp().map._combineEngine._earthInstance._scene
              .mapProjection,
            // eslint-disable-next-line no-undef
            !THING.Utils.getCurrentApp().map._isAmericaInWest
          );
        }

        let normalArray, worldNormal, inversedMatrix;
        if (geometry.attributes.normal) {
          normalArray = geometry.attributes.normal.values;
          // eslint-disable-next-line no-undef
          worldNormal = THING.MathUtils.createVec3();
          // eslint-disable-next-line no-undef
          inversedMatrix = THING.MathUtils.createMat4();
        }

        // eslint-disable-next-line no-undef
        const earthMode = THING.EARTH.Utils.getEarthMode();
        const position =
          earthMode === "3D"
            ? geometry.attributes.position.values
        ...
        const transform = rectangleGeometry.params.transform;
        const useMercatorUV = rectangleGeometry.params.useMercatorUV;
        const rec = rectangleGeometry.params.rec;
        const textureSize = rectangleGeometry.params.textureSize || [
          rec.east - rec.west,
          rec.north - rec.south,
        ];
        let tempValue = 0;
        ...
        for (let i = 0; i < position.length / 3; i++) {
          if (earthMode !== "3D") {
            tempValue = position[i * 3 + 1];
            position[i * 3] = tempPosition[0] = position[i * 3];
            position[i * 3 + 1] = tempPosition[1] = position[i * 3 + 2];
            position[i * 3 + 2] = tempPosition[2] = -tempValue;
          } else {
            tempPosition[0] = position[i * 3];
            tempPosition[1] = position[i * 3 + 1];
            tempPosition[2] = position[i * 3 + 2];
          }
          ...
          if (transform) {
            if (normalArray) {
              ...
              // eslint-disable-next-line no-undef
              inversedMatrix = THING.MathUtils.mat4.clone(
                transform.matrixInverse
              );
              // eslint-disable-next-line no-undef
              const localNormal = THING.MathUtils.transformDirection(
                worldNormal,
                inversedMatrix
              );
              normalArray[i * 3] = localNormal[0];
              normalArray[i * 3 + 1] = localNormal[1];
              normalArray[i * 3 + 2] = localNormal[2];
            }

            const localPos = transform.worldToLocal(tempPosition);
            position[i * 3] = localPos[0];
            position[i * 3 + 1] = localPos[1];
            position[i * 3 + 2] = localPos[2];
          }

          let coords;
          if (earthMode === "3D") {
            // eslint-disable-next-line no-undef
            coords = THING.EARTH.Utils.convertWorldToLonlat(tempPosition, "3D");
          } else {
            // eslint-disable-next-line no-undef
            coords = THING.EARTH.Utils.convertWorldToLonlat(
              tempPosition,
              earthMode
            );
          }
          if (useMercatorUV) {
            // eslint-disable-next-line no-undef
            coords = THING.EARTH.Utils.convertLonlatToWebMercator(coords);
          }

          const u = (coords[0] - rec.west) / textureSize[0];
          const v = 1 - (rec.north - coords[1]) / textureSize[1];
