...
define(['./defaultValue-81eec7ed', './Matrix2-a67cd174', './ArcType-fc72c06c', './GeometryOffsetAttribute-2bff0974', './BoundingRectangle-797c75a4', './Transforms-fe4a2875', './RuntimeError-8952249c', './ComponentDatatype-0f8fc942', './EllipsoidGeodesic-b13a05b4', './EllipsoidTangentPlane-03799ad1', './GeometryAttribute-013ac666', './GeometryInstance-8a3f1b2b', './GeometryPipeline-33c976f0', './IndexDatatype-2261ba8d', './PolygonGeometryLibrary-961e66ca', './PolygonPipeline-1f856727', './VertexFormat-7df34ea5', './_commonjsHelpers-3aae1032-26891ab7', './combine-3c023bda', './WebGLConstants-508b9636', './AxisAlignedBoundingBox-ca5327b9', './IntersectionTests-803c75e5', './Plane-8a0f393e', './AttributeCompression-e6061e33', './EncodedCartesian3-bf2feda0', './arrayRemoveDuplicates-9877adc7', './EllipsoidRhumbLine-d58fdd5b', './GeometryAttributes-32b29525'], (function (defaultValue, Matrix2, ArcType, GeometryOffsetAttribute, BoundingRectangle, Transforms, RuntimeError, ComponentDatatype, EllipsoidGeodesic, EllipsoidTangentPlane, GeometryAttribute, GeometryInstance, GeometryPipeline, IndexDatatype, PolygonGeometryLibrary, PolygonPipeline, VertexFormat, _commonjsHelpers3aae1032, combine, WebGLConstants, AxisAlignedBoundingBox, IntersectionTests, Plane, AttributeCompression, EncodedCartesian3, arrayRemoveDuplicates, EllipsoidRhumbLine, GeometryAttributes) { 'use strict';
          ...
          if (!defaultValue.defined(flatTexcoords)) {
            ...
            if (wall) {
              if ((textureCoordIndex / 2) % 2 === 0) {
                ...
                textureCoordinates[textureCoordIndex + 1] = 1;
                textureCoordinates[textureCoordIndex + bottomOffset2] = 1;
                textureCoordinates[textureCoordIndex + 1 + bottomOffset2] = 0;
              }
            }
   ...
   */
  PolygonGeometry.createGeometry = function (polygonGeometry, params) {
    ...
    if (extrude) {
      ...
      for (i = 0; i < polygons.length; i++) {
        ...
        groups.push({
          start: startIndex,
          count: topAndBottom.geometry.indices.length,
          materialIndex: materialIndex,
        });
        startIndex += topAndBottom.geometry.indices.length;
        materialIndex++;
        ...
        for (let k = 0; k < walls.length; k++) {
          ...
          groups.push({
            start: startIndex,
            count: wall.geometry.indices.length,
            materialIndex: materialIndex,
          });
          materialIndex++;
          startIndex += wall.geometry.indices.length;
        }
      }
    } else {
      for (i = 0; i < polygons.length; i++) {
        ...
        groups.push({
          start: startIndex,
          count: geometryInstance.geometry.indices.length,
          materialIndex: materialIndex,
        });
        materialIndex++;
        startIndex += geometryInstance.geometry.indices.length;
      }
    }

    let geometry;
    if (params) {
      geometry = GeometryPipeline.GeometryPipeline.combineInstancesForTHINGEARTH(
        geometries,
        params
      )[0];
    } else {
      geometry = GeometryPipeline.GeometryPipeline.combineInstances(geometries, params)[0];
    }
    ...
    // });
    const result = new GeometryAttribute.Geometry({
      attributes: attributes,
      indices: geometry.indices,
      primitiveType: geometry.primitiveType,
      boundingSphere: boundingSphere,
